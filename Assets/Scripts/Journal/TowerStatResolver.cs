...
public static class TowerStatResolver
{
    public static float Damage(TowerDefinition def) =>
        def.displayDamage > 0f ? def.displayDamage : ResolveDamage(def.fireAbilityId);
    ...
    public static float ShieldBonus(TowerDefinition def) => ResolveShieldBonus(def.fireAbilityId);
